...
public class GithubSpider {
  ...
  private static void crabIssue(String author, String repo, Repository repository) {
    ...
    try {
      ...
      int page = 1;
      while (true) {
        Document issueDOM = issueCon.data("page", String.valueOf(page)).data("per_page", "100")
            .get();
...
//                    Comment comment = new Comment();
  ...
  private static void crabRelease(String author, String repo, Repository repository) {
    ...
    try {
      Connection releaseCon = Jsoup.connect(releaseURL).ignoreContentType(true)
          .ignoreHttpErrors(true)
          .header("Authorization", "Bearer " + token);
      int page = 1;
      while (true) {
        Document releaseDOM = releaseCon.data("page", String.valueOf(page)).data("per_page", "100")
            .get();
  ...
  private static void crabCommit(String author, String repo, Repository repository) {
    ...
    try {
      ...
      int page = 1;
      while (true) {
        Document commitDOM = commitCon.data("page", String.valueOf(page)).data("per_page", "100")
            .get();
        ...
        commitObj.forEach(j -> {
          ...
          String commitDate = json.getJSONObject("commit").getJSONObject("committer")
              .getString("date");
